...
- Browser: real-browser no-reload verification is a manual step (single POST with no document reload; S4 reworked refresh.js to rebuild the `#results` container from the JSON response, failure leaves the page untouched)

M8 (Responsive at phone width) is **complete**. Completed slices: `public/style.css` only — no PHP/JS/markup or dependency changes (existing behaviors, refresh.js selectors, form structure, and table semantics all preserved). Global `box-sizing: border-box` on `*`/`::before`/`::after`; `overflow-wrap: anywhere` on `h1`, `.site-url a`, `.alert`, and both tables' `th`/`td` (long unbroken phrases and URLs wrap instead of overflowing); and a single `@media (max-width: 480px)` block: full-width `min-height: 44px` touch-target inputs (with `font-size: 1rem` to avoid iOS focus zoom) and buttons across the add/search/edit/refresh/delete controls, `.refresh-control` and `.actions` stacked column with stretch alignment, `.edit-link` full-width block, `table.keywords` (44/20/36%) and `table.history` (55/45%) `table-layout: fixed` with `width: 100%` so cells wrap within the viewport, reduced `body` top margin. No `overflow-x: hidden` hacks; all overflow sources fixed directly. Verified commands:
- Tests: `php tests/db_foundation_test.php` (38 checks, exit 0), `php tests/seed_trend_test.php` (37 checks, exit 0), `php tests/list_test.php` (38 checks, exit 0), `php tests/keyword_test.php` (30 checks, exit 0), `php tests/create_test.php` (39 checks, exit 0), `php tests/edit_delete_test.php` (45 checks, exit 0), `php tests/refresh_test.php` (34 checks, exit 0), `php tests/create_smoke_test.php` (41 checks, exit 0), `php tests/edit_delete_smoke_test.php` (112 checks, exit 0), `php tests/keyword_smoke_test.php` (26 checks, exit 0), `php tests/refresh_smoke_test.php` (35 checks, exit 0) — all exit 0, no warnings, no leftover temp files or stray processes
- Lint: `php -l` on all 28 PHP source/test/action files (0 failures; files unchanged by M8)
- Browser: real Chrome (headless CDP) at 375x812 device emulation against a fresh seeded scratch DB — no page-level horizontal overflow on list/detail/edit (`scrollWidth <= innerWidth` everywhere), media query active (full-width inputs/buttons), all buttons/inputs `.edit-link` `min-height: 44px`, `table-layout: fixed` on both tables (history table fits at 328px with all 30 rows), 120-char unbroken keyword wraps inside its cell and the detail `h1` with no overflow, and the full create/refresh/edit/delete round trips work at phone width (303 flows intact; scratch DB left at pristine seed)

S4 (Movement filtering) is **complete**. Completed slices: `src/list.php` (`minirank_parse_movement()` whitelist helper treating missing/array/unknown movement as inert, `minirank_search_keywords()` now takes an optional `$movement` filter applied in PHP to the already-computed trend rows — improved/declined/stable(real baseline only)/not_enough_history, `minirank_render_search()` renders a `movement` select combined with the `q` search in one GET form), `public/index.php` (parses `movement` from GET, passes it to the search, and wraps the list in a stable `<section id="results">` container showing either the table or the no-results message), `src/refresh.php` (`minirank_refresh()` accepts `$query` + `$movement`; `refreshed` stays the total keyword count while the `keywords` payload is the filtered subset, so the refresh total is separate from the filter count), `public/actions/refresh.php` (reads `q` + `movement` from the POST body), `public/js/refresh.js` (sends `q` + `movement`; on full success rebuilds the `#results` container from the filtered JSON via `createElement`/`textContent` so stale rows disappear, newly matching rows appear, values stay fresh, and no page reload occurs; status shows the total `refreshed` count), `public/style.css` (.search select + responsive rule), and `tests/filter_test.php` + `tests/filter_smoke_test.php`. Verified commands:
- Lint: `php -l src/list.php`, `php -l src/refresh.php`, `php -l public/index.php`, `php -l public/actions/refresh.php`, `php -l tests/filter_test.php`, `php -l tests/filter_smoke_test.php`
- Tests: `php tests/filter_test.php` (37 checks, exit 0, no warnings, no leftover temp files; includes the four movement categories adding up to the unfiltered total and partitioning the keyword set exactly)
- Tests: `php tests/filter_smoke_test.php` (37 checks, exit 0; HTTP: movement select + `<section id="results">` on the list page, each movement selects its exact set, malformed/array/unknown movement and search params leave the page untouched, search + movement combine, no-match renders the no-results message, filtered refresh JSON reports total `refreshed` separate from the filtered `keywords` count with no Location header, unfiltered refresh keeps M3 behavior, 405/403/500 all JSON with safe messages, unopenable-DB generic 500; no leftover temp files or stray processes)
- Regression: all 11 prior test files still exit 0 (db_foundation 38, seed_trend 37, list 38, keyword 30, create 39, edit_delete 45, refresh 34, create_smoke 41, edit_delete_smoke 112, keyword_smoke 26, refresh_smoke 35)
- Browser: real Chrome (headless CDP) audit of the refresh.js success and failure paths (32 checks, exit 0): filtered refresh rebuilds `#results` from the JSON with only matching rows and a fresh status showing the total `refreshed` count, no page reload (in-page window marker survives), wrong-CSRF 403 surfaces `Forbidden.`, and simulated network failure / non-JSON 500 / JSON-error 500 / malformed `ok:true` payload each leave the page untouched with the generic (or server) message; unfiltered refresh keeps M3 behavior (full table rebuilt), hostile phrases stay escaped in rebuilt rows
